...
import org.example.dockernetwork.Handler.MyWebSocketHandler;
...
public class TestServiceImpl implements TestService {
    ...
    public String HttpTest(String container1, String container2Ip) {
        ...
        try {
            ...
            MyWebSocketHandler.sendMessage("容器 " + container1 + " 访问 " + container2Ip + "，HTTP 状态码：" + statusCode);
            System.out.println("容器 " + container1 + " 访问 " + container2Ip + "，HTTP 状态码：" + statusCode);
    ...
    public String TCPTest(String container1, String container2, String container2Ip, int size, int time) {
        ...
        try {
            ...
            result.append("TCP 测试完成，退出代码：").append(exitCode).append("\n");

            // 停止容器内的 iperf3 server
            stopIperf3Server(container2);
    ...
    public String UDPTest(String container1, String container2, String container2Ip, String velocity, int size) {
        StringBuilder result = new StringBuilder();
        try {
            String serverCmd = "docker exec " + container2 + " iperf3 -s -B " + container2Ip;
            ...
            int exitCode = clientProcess.waitFor();
            result.append("UDP 测试完成，退出代码：").append(exitCode).append("\n");
    ...
    @Override
    public String PingTest(String container1, String container2Ip, int size, int num) {
        StringBuilder result = new StringBuilder();
        try {
            String cmd = "docker exec " + container1 + " ping -c " + num + " -s " + size + " " + container2Ip;
            ProcessBuilder processBuilder = new ProcessBuilder("bash", "-c", cmd);
            Process process = processBuilder.start();
    ...
    /**
     * 辅助方法：停止容器内运行的 iperf3 -s 进程
     */
    private void stopIperf3Server(String containerName) {
        try {
            String stopCmd = "docker exec " + containerName + " pkill -f 'iperf3 -s'";
            ProcessBuilder stopPB = new ProcessBuilder("bash", "-c", stopCmd);
